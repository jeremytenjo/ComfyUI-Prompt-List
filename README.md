...
2. Set inputs:
   ...
   - `default_negative_prompt` (optional): fallback negative prompt used when a block's negative is empty
   - `prompt_positive_suffix` (optional): text appended literally to every positive output prompt
...
- `prompt_positive_suffix`: `, high detail`

Outputs:

- `positive`: `["prompt A, high detail", "prompt B, high detail"]`
- `negative`: `["", ""]`
...
When `default_negative_prompt` is set, any empty negative value (missing or blank `negative:`) is replaced with that fallback.
